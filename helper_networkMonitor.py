...
def print_http(server):
    ...
    result = (f"{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")} HTTP URL: {http_url}, Status Code: {http_server_response_code if http_server_response_code is not None else 'N/A'}")
...
def print_https(server):
    ...
    result = (f"{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")} HTTPS URL: {https_url}, Status Code: {https_server_response_code if https_server_response_code is not None else 'N/A'}, Description: {description}")
...
def print_udp(server):
    ...
    result =  f"{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")} UDP Server: {udp_port_server}, Port: {udp_port_number}, UDP Port Status: {udp_port_status}, Description: {udp_port_description}"
...
def print_tcp(server):
    ...
    result = f"{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")} TCP Server: {tcp_port_server}, Port: {tcp_port_number}, TCP Port Status: {tcp_port_status}, Description: {tcp_port_description}"
...
def print_ntp(server):
    ...
    result = f"{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")} NTP {ntp_server} is up. Time: {ntp_server_time}" if ntp_server_status else f"NTP {ntp_server} is down."
...
def print_dns(server):
    ...
    for dns_query, dns_record_type in dns_queries:
        ...
        result = f"{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")} DNS Server: {dns_server}, Status: {dns_server_status}, {dns_record_type} Records Results: {dns_query_results}"
...
def print_icmp(server):
    """
    """
    result = f"{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")}\n" + traceroute(server['server'])
